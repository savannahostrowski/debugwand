...
class TestGetAndSelectPod:
    ...
    def test_single_pod_returned(self, mock_select: MagicMock, mock_get_pods: MagicMock):
    ...
    @patch('debugwand.operations.get_pods_for_service')
    @patch('debugwand.operations.select_pod')
    def test_multiple_pods_calls_select(self, mock_select: MagicMock, mock_get_pods: MagicMock):
...
class TestGetAndSelectProcess:
    ...
    def test_no_pid_provided_calls_select(self, mock_select: MagicMock, mock_list: MagicMock):
...
class TestPrepareDebugpyScript:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data="debugpy.listen({PORT})\nif {WAIT}: debugpy.wait_for_client()")
    @patch('tempfile.NamedTemporaryFile')
    def test_script_preparation_with_defaults(self, mock_temp: MagicMock, mock_file: MagicMock):
    ...
    @patch('builtins.open', new_callable=mock_open, read_data="debugpy.listen({PORT})\nif {WAIT}: debugpy.wait_for_client()")
    @patch('tempfile.NamedTemporaryFile')
    def test_script_preparation_custom_values(self, mock_temp: MagicMock, mock_file: MagicMock):
